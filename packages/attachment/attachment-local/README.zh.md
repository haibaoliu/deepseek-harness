---
description: "DSH_HOME 下附加图片、文件与文档的本地存储，供用户与维护者选择或排查图片及文档附件的存放位置。"
...
在运行 DSH 的机器上，把图片与通用文件附件持久存储到 `DSH_HOME` 下。图片经过校验、针对模型请求完成规范化并按路由缓存；通用文件不设准入限制，按字节原样保存；文本型文档经过校验、按字节原样存储，并抽取为普通文本送入模型。即使上传时使用不同显示名称，相同字节也只存储一次；读取会校验文件长度与内容，之后收紧限制也不会让已接纳的图片不可读。随附的 `dsh` 组合无需配置即可使用本包。对象仅限本机，并且永远不会自动删除。
...
| `maxDocumentBytes` | `25 MiB` | 单个文档接受的最大编码源字节数 |
| `maxDocumentsPerMessage` | `10` | 单条提交消息接受的最大文档数量 |
| `maxMessageDocumentBytes` | `100 MiB` | 单条提交消息接受的最大文档编码源字节总数 |
...
附加的图片保存在本机的 `<DSH_HOME>/attachments/v1` 下。已存储的图片与文档永远不会被自动删除，相同字节只会存储一份，之后收紧限制也绝不会让已保存的图片不可读。如果你的图片需要能从另一台机器读取，本包并不合适。
...
附加 Markdown、PDF、DOCX 或 PPTX 文档后，会先检查字节上限与声明媒体类型是否与容器字节一致，再抽取纯文本。原始字节按内容寻址原样存储，抽取出的文本以普通文本块进入提示词，同时记录一条对模型隐藏的引用用于渲染与回放；编码后的文档本身绝不会提交给模型。Markdown 按 UTF-8 解码，PDF 文本通过 `unpdf` 抽取，DOCX/PPTX 文本在 `fflate` 解压后从 OOXML 文本 run 中抓取。

### 可能出什么问题

附加图片时可能被拒绝：格式不受支持、超出字节、像素或单边尺寸限制，或者字节与声明类型不符。附加文档时可能因字节为空、超出文档字节限制、与声明容器不符，或无法解码与抽取文本而被拒绝。之后读取时，磁盘上被删除或损坏的图片或文档会以明确错误失败。每个失败都带有稳定错误码，客户端与协议适配器可以用自己的措辞解释。
...
- **文档只变成文本，绝不上行字节。** 准入会按魔数校验声明的容器，按内容寻址原样存储原始字节，并抽取纯文本；模型只会收到该文本，因此提供方是否支持某种格式无关紧要。
...
文档按原样存放在与规范化图片相同的 `<DSH_HOME>/attachments/v1/objects/<sha256-prefix>/<sha256>` 树中，并通过相同的暂存写入与 fsync 链发布。准入允许每条消息最多 10 个文档与 100 MiB 源字节；单个文档最多 25 MiB。Markdown 必须能按 UTF-8 解码；PDF 必须以 `%PDF-` 开头；DOCX 与 PPTX 必须是携带 `word/document.xml` 或 `ppt/presentation.xml` 的 ZIP 容器。`saveDocument` 同时返回持久引用与抽取出的文本，`readDocument` 会重新校验摘要与记录的字节数。
...
| [`src/document.ts`](src/document.ts) | Markdown、PDF、DOCX 与 PPTX 的文档容器校验与纯文本抽取 |
...
本包通过请求描述符间接影响模型。执行文件系统可以映射宿主对象时，模型会随请求字节看到每张图片的身份、尺寸、媒体类型、只读进程路径、可写副本扩展名与规范化警告。通用文件会投影为指出文件身份与只读进程路径的文本句柄；无法映射时，句柄会说明当前执行环境不能读取该文件。文档以抽取出的纯文本形式、作为普通 `text` 块到达模型；编码后的文档本身绝不会提交给模型。
